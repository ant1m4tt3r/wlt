...
public class Utilities {
	 ...
	 * @return
	 */
	public static Object randomSelect(Object first, Object second, Object... objects) {
		if (first == null || second == null)
			throw new NullPointerException("\"first\" and \"second\" parameters can't be null.");

		int length = objects == null ? 0 : (objects.length + 2);
		Object[] objs = new Object[(int) length];
		objs[0] = first;
		objs[1] = second;
	...
	/**
	 * This method actually returns an Integer, 1 = true, 0 = false This is done
	 * so beacuse if an exception it's catched in the parsing, it will return an
	 * error constant {@code ERROR_INTEGER}
	 * 
	 */
	...
	// Interface Utilities
	/**
	 * this method receives any {@code Component} and centralizes it on the
	 * screen
	 * 
	 * @param frame
	 */
	...
	/**
	 * 
	 * @return phrase containing the date in the brazilian format Eg.:"1 de
	 *         julho de 2016."
	 */
	...
	/**
	 * Verifys if the <code>String date</code> is valid, checking day, months,
	 * year and bissextile verification. If the date also contains time, verifys
	...
	public static final Date parseToDateDMY(String date) {
		...
		date = date.trim();
		if (!date.matches("[0-9]+/[0-9]+/[0-9]+ [0-9]*:[0-9]*") && (!date.matches("[0-9]+/[0-9]+/[0-9]+"))
				&& !date.matches("[0-2]+/[0-9]") && (!date.matches("[0-2]+/[0-9]+[0-9]")))
