...
public class SheetHandler implements AbstractHandler {
	...
	public Integer getTitleColum(String pesquisa) throws Exception {
		...
		throw new Exception("Não foi encontrado o titulo '" + pesquisa + "' na aba: '" + sheet.getSheetName()
				+ "' em: " + file.substring(file.lastIndexOf("\\") + 1));
	...
	public String getCellValue(Integer col, Integer line) throws Exception {
		...
		try {
			...
			throw new Exception(String.format(
					"Não foi possível fazer a leitura da célula de coluna: %s e linha: %s no arquivo: %s", col,
					line + 1, file.substring(file.lastIndexOf("\\") + 1)));
		}
		return null;
	}
	...
	public String getCellValueString(Integer col, Integer line) throws Exception {
		...
		try {
			...
			throw new Exception(String.format(
					"Não foi possível fazer a leitura da célula de coluna: %s e linha: %s e no arquivo: %s", col,
					line + 1, file.substring(file.lastIndexOf("\\") + 1)));
		}
		return null;
	}
